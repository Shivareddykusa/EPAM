...
class MLP_TF:
    ...
    def predict(self, x):
        x = tf.constant(x, dtype=tf.flot32)
    ...
    def train(self, x, y, iterations, lr):
        ...
        for i in range(iterations):
            with tf.GradientTape() as tape:
                z1, a1, z2, a2 = self.forward_propagation(tf.transpose(x))
            ...
            self.w1.assign_sub(lr * dw1)
            self.b2.assign_sub(lr * db2)
            self.b1.assign_sub(lr * db1)
